...
def sanitize_label(label):
    """Sanitize labels for Neo4j compatibility, only if necessary."""
...
""" def determine_entity_label(entity):
    ...
    # Early filtering: skip labeling as cell or anatomy if terms indicate something else
    non_cell_process_keywords = {
        "activation", "migration", "differentiation", "proliferation",
        "response", "infiltration", "signaling", "secretion", "maturation",
        "apoptosis", "physiology", "function", "recruitment"
    }

    anatomy_exclusion_keywords = {
        "injury", "impairment", "abnormality", "dysfunction", "symptom",
        "fog", "condition", "syndrome", "lesion", "damage"
    }

    cell_keywords = {"cell", "cells", "cyte", "cytes", "blast", "blasts",
        "fibroblast", "macrophage", "neuron", "astrocyte", "oligodendrocyte",
        "lymphocyte", "monocyte", "t cell", "b cell", "nk cell",
        "dendritic cell", "stem cell", "progenitor", "hepatocyte", "myocyte",
        "keratinocyte", "germ cell", "oocyte", "spermatocyte", "erythrocyte",
        "thrombocyte", "megakaryocyte", "microglia", "schwann cell",
        "retinal ganglion", "osteoblast", "osteocyte", "chondrocyte"
    }

    anatomy_keywords = {
        "brain", "neuron", "olfactory", "bulb", "epithelium", "barrier",
        "plate", "tissue", "organ", "ventricle", "cortex", "hippocampus",
        "amygdala", "hypothalamus", "pituitary", "thalamus", "spinal",
        "nervous system", "axon", "dendrite", "glomerulus", "ganglion",
        "bone", "skin", "muscle", "kidney", "lung", "heart", "spleen",
        "liver", "cranium", "membrane", "tract", "artery", "capillary",
    ...
    # Continue with other label detection (Protein, Biological Process, etc.)
    protein_keywords = {"protein", "receptor", "enzyme", "cytokine",
        "interleukin", "chemokine", "ligand", "kinase", "channel",
        "transporter", "antibody", "peptide", "growth factor"}

    if any(kw in entity_lower for kw in protein_keywords):
        return "Protein", "PR", None

    # Gene heuristic check
    if entity_gene.isupper() and len(entity_gene) <= 6:
        label, namespace, ontology_id = query_hgnc_fuzzy(entity_gene)
        if label:
            return sanitize_label(label), sanitize_label(namespace), ontology_id
    ...
    phenotype_keywords = ["phenotype", "clinical", "abnormal", "trait", "symptom"]
    ...
    process_keywords = ["activation", "process", "pathway", "signaling", "regulation", "metabolism"]
    ...
    chemical_keywords = ["acid", "amine", "hormone", "neurotransmitter", "melanin", "lipid", "compound", "chemical", "drug", "anti-inflammatory"]
...
def load_and_tag(file_path: str, source_tag: str) -> pd.DataFrame:
    """
    Detect delimiter (comma/semicolon/etc), load the CSV,
    fill missing columns, set its source tag.
    Ensures columns: Subject, Object, Predicate, URL,
...
def main():
    ...
    fulltext_fp = os.path.join(base, "Triples_From_Full_Text_CBM_cleaned.csv")
...
if __name__ == "__main__":
    main()
#%%
###query to clean db at the end
"""
-) MATCH (n) where toLower(n.name) contains "cell" set n:Cell remove n:Chemical remove n:Unknown return n
-) 

"""
#ask gpt to correct node labels
# prompt:

"""
Here’s a more precise, structured prompt you can feed to ChatGPT (or any LLM) to get back a corrected CSV of entity→node‐label mappings:

> **Prompt:**  
> I have a CSV with two columns, **Entity** and **Current_Label**, listing biological entities and their existing labels. Please review each row, correct any misassignments, and assign the most appropriate label from this controlled vocabulary:  
> 
...
> - Protein  
> 
> **Output** a new CSV (or tabular JSON) with exactly three columns:  
> 
> 1. **Entity** (unchanged)  
> 2. **Current_Label** (unchanged)  
> 3. **Corrected_Label** (one of the nine labels above)  
> 
> Use other relevant label if none of the nine categories apply. Do not add any extra columns or commentary.  

"""
#%% Clean correct Unknown labels (get file from gpt)
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
#!/usr/bin/env python3
import csv
...
def sanitize_label(label: str) -> str:
    """Turn arbitrary category names into valid Neo4j label names."""
...
MAPPING_CSV    = "corrected_mapping_2.csv"

driver = GraphDatabase.driver(NEO4J_URI, auth=(NEO4J_USER, NEO4J_PASSWORD))

def reassign_label(tx, original, corrected_name, category):
    cat_label = sanitize_label(category)
    # First, try to remove and relabel, and return how many nodes were touched
    result = tx.run(
        f"""
        MATCH (n:Unknown {{name: $original}})
        WITH n
        REMOVE n:Unknown
    ...
    with open(MAPPING_CSV, newline='', encoding='utf-8') as csvfile:
        ...
        for row in reader:
            updated = session.write_transaction(
                reassign_label,
                row['original'],        # raw original name, with spaces/punctuation
                row['corrected_name'],  # raw corrected name, with punctuation
                row['category']         # will be sanitized into a label
            )
            print(f"{row['original']!r} → {row['category']} : {updated} node(s) updated")
            total += updated

    print(f"\n✔ Done. Total nodes updated: {total}")

driver.close()

# %% use openai to correct labels
...
Connects to Neo4j and uses GPT-4 to confirm or correct each node's current_label.
Configuration (Neo4j creds & OpenAI key) is loaded from config.ini.
"""
...
CONTROLLED_VOCAB = [
    "Anatomical_Structure", "Biological_Process", "Cell",
    "Cell_Phenotype", "Chemical", "Disease",
    "Gene", "Phenotype", "Protein"
]

def review_label(entity: str, current_label: str) -> str:
    prompt = (
        "You are an expert in biological entity classification.\n\n"
        f"Entity: \"{entity}\"\n"
        f"Current label: \"{current_label}\"\n\n"
        f"Controlled Vocabulary: {', '.join(CONTROLLED_VOCAB)}\n\n"
        "Choose the single most appropriate label. "
        "If the current label is correct, return it as-is. "
        "If none fit, propose a new label. Respond with exactly one label."
    )
    try:
        ...
        return resp.choices[0].message.content.strip() or current_label
    except Exception as e:
        print(f"[ERROR] GPT API error for '{entity}': {e}", file=sys.stderr)
...
def main():
    ...
    try:
        ...
        print(f"ERROR: Missing section in config.ini: {e}", file=sys.stderr)
        sys.exit(1)

    neo4j_uri      = neo4j_conf.get('uri')
    neo4j_user     = neo4j_conf.get('user')
    ...
    if not all([neo4j_uri, neo4j_user, neo4j_password, openai_api_key]):
        print("ERROR: Please fill in all values in config.ini", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        with driver.session() as session:
            results = session.run("""
                MATCH (n)
                WHERE exists(n.name) AND exists(n.current_label)
                RETURN id(n) AS id, n.name AS entity, n.current_label AS current_label
            """)
            for record in results:
                nid   = record["id"]
                ent   = record["entity"]
                curr  = record["current_label"]

                new = review_label(ent, curr)
                if new != curr:
                    session.run(
                        "MATCH (n) WHERE id(n) = $id SET n.current_label = $new",
                        id=nid, new=new
                    )
                    print(f"[UPDATED] '{ent}': {curr} → {new}")
                else:
                    print(f"[UNCHANGED] '{ent}' remains '{curr}'")
